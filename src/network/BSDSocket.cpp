          ...
          m_receive_queue( config.allocator ? *config.allocator : core::memory::default_allocator() )
    {
        ...
        if ( m_config.ipv6 )
        {
            ...
            else
            {
                size_t maxlen = 50;
                char buff[maxlen];
                inet_ntop(AF_INET6, &(sock_address.sin6_addr), buff, maxlen);
                printf("bind ipv6 addr = %s\n", buff);
            }
        }
        else
        {
            ...
            else
            {
                size_t maxlen = 30;
                char buff[maxlen];
                inet_ntop(AF_INET, &(sock_address.sin_addr), buff, maxlen);
                printf("bind ipv4 addr = %s\n", buff);
            }
        }
