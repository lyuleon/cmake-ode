...
typedef protocol::SequenceBuffer<game::Input> InputSequenceBuffer;
...
struct SyncInternal
{
        ...
        : packet_factory(allocator)
    {
        ...
        inputBuffer = CORE_NEW(allocator, InputSequenceBuffer, allocator, NumStateUpdates);
        Reset(mode_data);
    }
    ...
    InputSequenceBuffer* inputBuffer;
};
...
SyncDemo::SyncDemo(core::Allocator& allocator)
{
    ...
    m_settings->deterministic = false;
...
void SyncDemo::NoDelayUpdate()
{
    ...
    game::Input* input_ptr = m_sync->inputBuffer->Insert(m_sync->send_sequence);
    *input_ptr = local_input;
    ...
    StateUpdateUncompressed right_past_update;
    bool rightModification = false;
    ...
    while (true)
    {
        ...
        if (!m_sync->disable_packets && state_packet->state_update.sequence > m_sync->reci_sequence)
        {
            //use the latest server state to update client in the past!!
            rightModification = true;
            m_sync->reci_sequence = state_packet->state_update.sequence;
            right_past_update = state_packet->state_update;
        }
        m_sync->packet_factory.Destroy(packet);
    }


    if(rightModification)
    {
        ApplyStateUpdateUncompressed(*m_internal->simulation[1].game_instance, right_past_update, m_sync->position_error, m_sync->orientation_error);
        //render right side from m_sync->reci_sequence
        for (size_t i = m_sync->reci_sequence; i < m_sync->send_sequence-1; ++i)
        {
            game::Input* inPtr = m_sync->inputBuffer->Find(i);
            if (inPtr)
            {
                // predict right simulation
                CubesUpdateConfig update_config;
                update_config.sim[1].num_frames = 1;
                update_config.sim[1].frame_input[0] = *inPtr;
                m_internal->Update(update_config);
            }
            else
            {
                printf("id= %d input was not there!\n", i);
            }
        }
    }
    ...
    update_config.sim[1].num_frames = 1;
    update_config.sim[1].frame_input[0] = local_input;
    m_internal->Update(update_config);
